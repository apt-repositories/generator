...
export const getPackagesRawXZ = async (
	...
	if (packagesResponse.statusCode === 200) {
		...
		// );
		return Promise.all([
			getPackagesTextFromXZ(responsePayload),
			packagesResponse.headers,
		]).catch((error) => {
			process.stderr.write(
				`  ! Response for '${packagesUrl}' failed to parse!\n`,
			);
...
export const getPackagesRawGZ = async (
	...
	if (packagesResponse.statusCode === 404) {
		...
		throw new Error(
			`! Received HTTP 404 response for '${packagesUrl}'. Failed.`,
		);
	}
...
export const parsePackages = function* (
	packagesRaw: string,
