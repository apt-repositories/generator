...
import {
	errorToString,
	unknownToError,
} from "@oliversalzburg/js-utils/errors/error-serializer.js";
...
const serializeConfiguration = () => {
	const tasks: MirrorConfiguration[] = [];
	for (const [id, repository] of Object.entries(repositories)) {
		...
		for (const release of repository.releases) {
			for (const component of repository.components) {
				if (
					(repository.excludedComponents?.[release] ?? []).includes(component)
				) {
					continue;
				}

				const isEmpty = (repository.emptyComponents?.[release] ?? []).includes(
					component,
				);
				const isGzip = (repository.gzipComponents?.[release] ?? []).includes(
					component,
				);
				...
				tasks.push({
					...
					rootRelease: release.includes("-")
						? release.substring(0, release.indexOf("-"))
						: release,
...
const toObservableTasks = (completedTasks: Array<MirrorConfiguration>) => {
	...
	for (const task of completedTasks) {
		...
		if (!tasks.has(key)) {
			tasks.set(key, [] as MirrorConfiguration[]);
		}
...
const main = async () => {
	...
	const runTask = async (
		...
		for (const [_index, task] of taskIterator) {
			...
			if (!task.isEmpty) {
				for await (const repositoryPackage of parsePackages(
					packagesText,
					headers,
				)) {
	...
	const [, durationGenerate] = OBSERVE_ONLY
		? [0, 0]
		: await measureAsync(() =>
				Promise.all(Array(5).fill(tasks.entries()).map(runTask)),
			);
...
const entrypoint = async () => {
	...
	process.stderr.write(
		`Completed successfully after ${formatMilliseconds(duration)}.\n`,
	);
};
